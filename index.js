...
function filterAndAdd(mySet, arr) {
  for(const filter of mySet) {
    if (typeof filter === 'number') {
      mySet.delete(filter)
    }
    for(const filter1 of arr) {
      mySet.add(filter1)
    }
  }
  return mySet
...
function setToArray(mySet) {
  ...
  // Фільтруємо масив, залишаючи лише рядкові елементи.
  const stringElements = arr.filter((item) => typeof item === "string");

  // Сортуємо рядкові елементи в алфавітному порядку.
  stringElements.sort();
